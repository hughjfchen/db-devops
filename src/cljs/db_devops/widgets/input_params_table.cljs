   ...
   (when-let [inputs (map-indexed vector @data-atom)]
     (let [table-spec (into [["源主机名" :SRCHOSTNAME] ["源实例名" :SRCINSTANCENAME] ["源库" :SRCDBNAME] ["目标主机名" :TGHOSTNAME] ["目标实例" :TGINSTANCENAME] ["目标库" :TGDBNAME]] additional-columns)
       ...
       [sa/Table (merge {:celled true :size "small"} options)
        ...
        [sa/TableBody
         (for [input inputs]
           (into ^{:key (first input)}[sa/TableRow]
                 (if row-delete-button? (concat (map (fn [y] [sa/TableCell (if (keyword? (second y))  ((second y) (second input)) ((second y) (r/cursor data-atom [(first input)]) (first input) v-errors))]) table-spec) (list [sa/TableCell
                                                                                                                                                          [sa/Button {:icon true
                                                                                                                                                                      :on-click #(swap! data-atom delete-element (first input))}
                                                                                                                                                           [sa/Icon {:name "minus"}]]]))
                     (map (fn [y] [sa/TableCell (if (keyword? (second y))  ((second y) (second input)) ((second y) (r/cursor data-atom [(first input)]) (first input) v-errors))]) table-spec))))]])))
